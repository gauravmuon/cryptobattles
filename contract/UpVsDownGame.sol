...
contract UpVsDownGameV2 is Ownable {
    ...
    function distribute(
        ...
        if (
            ...
            BetGroup storage returnGroup = round.downBetGroup.bets.length == 0
                ? round.upBetGroup
                : round.downBetGroup;
        ...
        for (uint i = winners.distributedCount; i < to; i++) {
            uint256 winnings = ((((winners.bets[i] * 100) / winners.total) *
                dist.totalMinusFee) / 100);
        ...
        if (winners.distributedCount == winners.bets.length) {
            sendEther(
                feeAddress,
                dist.fee + dist.totalMinusFee - winners.totalDistributed
            );
            clearPool(poolId);
        }
    }
